...
const (
	...
	mgoconnstring         string = "192.168.33.10:27017"       // mongodb连接字符串
...
var setting = func() config.Configer{
	...
	os.MkdirAll( JS_TEMP, 0777 )                               // Surfer-Phantom下载器：js文件临时目录
	os.MkdirAll( FILE_DIR, 0777 )                              // 文件（图片、HTML等）结果的输出目录
	os.MkdirAll( TEXT_DIR, 0777 )                              // excel或csv输出方式下，文本结果的输出目录
